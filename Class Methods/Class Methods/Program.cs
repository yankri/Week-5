...
namespace Class_Methods
{
    class Program
    {
        ...
        {
            Console.WriteLine("Enter an integer to scale the house.");
            int scale = int.Parse(Console.ReadLine());

            Console.Clear();
          
            for (int row = 1; row < scale; row++) //borrowed this code from the CSharpPlayersGuide Ch 12 print a pyramid exercse
            {
                for (int space = scale - row; space > 0; space--)
                {
                    Console.Write(" ");  //originally used dashes here to make the spaces visible while testing 
                }
            ...
            {
                Console.Write(string.Concat(Enumerable.Repeat("|", 1)) + string.Concat(Enumerable.Repeat(" ", scale * 2 - 3)) + string.Concat(Enumerable.Repeat("|", 1)) + "\n"); //This made printing the body of the house much easier. Scale is times 2 to match the width of the roof pyramid. The minus 3 accounts for the spaces and 0 based indexing. 
            }

            Console.WriteLine(string.Concat(Enumerable.Repeat("=", scale * 2 - 1))); //scale is times 2 so that it is the same width as the pyramid. The minus one is to account for 0 based indexing in the pyramid creation
        }
